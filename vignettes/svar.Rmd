...
The authors of `sparsevar` are [...] and the R package is mantained by Simone Vazzoler.
...
Then, using a function included in the package, we simply generate a VAR$(2)$ process
```{r, cache = TRUE}
set.seed(1)
sim <- simulateVAR(N = 20, p = 2)
```

and we estimate the VAR matrix using
```{r, cache = TRUE}
